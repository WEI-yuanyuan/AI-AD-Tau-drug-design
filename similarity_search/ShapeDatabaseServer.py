...
InterfaceData = """\
!BRIEF [-shapeOnly | -chemff <color forcefield>] [-hostname <host>] [-dbase <db>] [-port <port>]
!PARAMETER -dbase
  !TYPE string
  !REQUIRED true
...
def setup_server(
    ...
    itf = oechem.OEInterface(InterfaceData, [
        ...
        '-port', str(port),
        '-hostname', hostname,
        '-shapeOnly', str(shapeOnly).lower(),
    ])
    
    # create server
    server = AsyncXMLRPCServer((hostname, port),
    ...
    print(f"Server is ready on {hostname}:{port}")
...
if __name__ == '__main__':
    ...
    sys.exit(setup_server(sys.argv))
